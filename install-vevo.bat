...
title Vevo 1.5 Installer

echo Welcome to the Vevo 1.5 Installer!
echo.

set "INSTALL_DIR=%cd%"
set "MINICONDA_DIR=%UserProfile%\Miniconda3"
set "ENV_DIR=%INSTALL_DIR%\env_vevo"
...
echo Vevo 1.5 has been installed successfully!
echo To start Vevo 1.5, please run 'run-vevo.bat'.
echo.
pause
exit /b 0
...
uv pip install torch==2.0.1 torchvision==0.15.2 torchaudio==2.0.2 --upgrade --index-url https://download.pytorch.org/whl/cu118 || goto :error
uv pip install -r "%INSTALL_DIR%\req\requirements_vevo_1.5.txt" || goto :error
